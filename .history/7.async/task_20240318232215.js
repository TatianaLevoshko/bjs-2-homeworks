class AlarmClock {
	...
	start() {
		...
		this.intervalId = setInterval(() => {
			// Текущее время
			const currentTame = this.getCurrentFormattedTime();

			// Перебирает все звонки
			this.alarmCollection.forEach(alarm => {
				// Gроверяйте на возможность его запуска: *свойство `time` совпадает с текущим временем, и звонок может выполняться (в свойстве `canCall` находится `true`
				if (alarm.time === currentTame && alarm.canCall) {
					alarm.canCall = false;
					alarm.callback();
				}
			});
